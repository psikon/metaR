...
#' Generate a parser function for converting taxonomyReportDB objects to a biom object
#'  
#' @param id a field that can be used to id a table
#' @param format name and version of the current biom format
#' @param static URL providing format details
#' @param type Table type (a controlled vocabulary) 
#' @keywords internal
biom_generator <- function(id = NULL, 
  ...
  to_biom <- function(db_list) {
    # initiliaze data and row fields
    ...
    foreach(db = iter(db_list), sample = icount(length(db_list))) %do% {
      ...
      foreach(taxid = iter(names(count)), taxcount = iter(count)) %do% {
        ...
        # the matrix values start with zero
...
#' generate OTU tables for the row statement of biom class
#' @keywords internal
...
#' generate column statements for biom class
#' @keywords internal
...
#' Convert a list of taxonomyReportDB objects into a biom object.
#' 
#' @usage to_biom(db_list)
#' @param db_list List of \code{\linkS4class{taxonomyReportDb}} objects.
#' @export
